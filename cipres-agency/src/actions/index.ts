...
const resendEmail = process.env.RESEND_EMAIL; // where you want to receive messages (your inbox)
const fromEmail = process.env.FROM_EMAIL;     // verified sender in Resend (e.g. onboarding@resend.dev)
...
// Initialize Redis with Upstash (optional)
// const redis = new Redis({
...
export const server = {
  send: defineAction({
    ...
    handler: async ({ name, email, message, company }, context) => {
      ...
      // 2) (Optional) Rate limiting with Redis - left commented out
      // const ip =
      ...
      // 3) Send email
      const { data, error } = await resend.emails.send({
        from: `Benelux <${fromEmail}>`, // MUST be a verified sender
        to: resendEmail,               // your inbox
        replyTo: email,               // reply will go to the user who filled the form
        subject: `New Contact from ${name}`,
        html: `
          <h2>New Contact Message</h2>
          <p><strong>Name:</strong> ${name}</p>
